...
llm = LiteLlm(model="ollama_chat/gemma-3-4b-it-GGUF")

def create_pokemon_agent():
    pokemon_agent_instruction = """
    ...
    return Agent(
        ...
        instruction=pokemon_agent_instruction,
        tools=MCPToolset(
            connection_params=StdioConnectionParams(
                server_params=StdioServerParameters(
                    command='uv',
                    args=['run', 'mcp_tool.py'],
                ),
            ),
...
def create_pokemon_team_agent(pokemon_agent: Agent):
    ...
    You have a specialized sub-agent named 'pokemon_agent', that handles retrieving a pokemon. Delegate to it whenever you need a pokemon.
    ...
    return Agent(
        name="pokemon_team_agent", # Give it a new version name
        model=llm,
